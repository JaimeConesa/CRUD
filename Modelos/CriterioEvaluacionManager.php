...
class CriterioEvaluacionManager {
    ...
    // Buscar por ID
    public function findById(int $id): ?CriteriosEvaluacion {
        try {
            ...
            return new CriteriosEvaluacion($row->id, $row->nombre, $row->asignatura_id);
    ...
    public function findAll(): array {
        try {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_OBJ)) {
                $criterios[] = new CriterioEvaluacion($row->id, $row->nombre, $row->asignatura_id);
            }
    ...
    public function save(CriterioEvaluacion $criterio): bool {
        try {
            if ($criterio->getId()) {
                // Actualizar
                $query = "UPDATE criterios_evaluacion SET nombre = :nombre, asignatura_id = :asignatura_id WHERE id = :id";
                $stmt = $this->conexion->prepare($query);
                $id = $criterio->getId();
                $stmt->bindParam(":id", $id, PDO::PARAM_INT);
            } else {
                // Insertar
                $query = "INSERT INTO criterios_evaluacion (nombre, asignatura_id) VALUES (:nombre, :asignatura_id)";
                $stmt = $this->conexion->prepare($query);
            }

            $nombre = $criterio->getNombre();
            $asignatura_id = $criterio->getId_asignatura();
